...
class TestDdpWithRpc(MultiProcessTestCase):
    ...
    @dist_init
    def _master_process(self, ddp_mode: DdpMode, grads_checker):
        ...
        self.do_test_on_master(
            ddp_mode, remote_em_rref, remote_net_rref, grads_checker
        )
        dist.destroy_process_group(process_group_for_ddp)
        ...
        for idx, trainer_rref in enumerate(trainer_rrefs):
            futures.append(
                _remote_method_async(
                    Trainer.do_backward, trainer_rref, training_examples[idx]
                )
            )
        dist_gradients = []
        for future in futures:
            dist_gradients.append(future.wait())
        grads_checker(dist_gradients)

    def _do_test(self, ddp_mode: DdpMode, grads_checker):
        if self.rank == MASTER_RANK:
            self._master_process(ddp_mode, grads_checker)
    ...
    def test_backward_no_ddp(self):
        def check_grads(trainer_grads):
            pass

        self._do_test(DdpMode.NONE, check_grads)

    def test_backward_ddp_outside(self):
        def check_grads(trainer_grads):
            pass

        self._do_test(DdpMode.OUTSIDE, check_grads)

    def test_backward_ddp_inside(self):
        def check_grads(trainer_grads):
            pass

        self._do_test(DdpMode.INSIDE, check_grads)
